...
- **Наблюдение по флейкости (для тикета 23).** При полном параллельном прогоне
  наборов дважды наблюдались единичные падения, не воспроизводимые изолированно:
  `follow-ups.integration.test.ts > enforces lifecycle transitions` и
  `beforeAll` в `atomic-corrections-history.integration.test.ts`
  (`create_client` под нагрузкой). Повторный полный прогон — 86 файлов,
  580 тестов, 0 падений. Похоже на давление на соединения/пул при параллельном
  запуске 86 integration-файлов, а не на дефект домена; release gate (тикет 23)
  должен либо ограничивать параллелизм integration-наборов, либо ретраить
  инфраструктурные сбои, чтобы флейк не блокировал релиз ложно.
